import { Code } from './codeType';

export const lczList: { [K in Code]: string[] } = {
	ad: ['Andorra'],
	ae: ['United Arab Emirates', 'UAE'],
	af: ['Afghanistan'],
	ag: ['Antigua and Barbuda'],
	al: ['Albania'],
	am: ['Armenia'],
	ao: ['Angola'],
	ar: ['Argentina'],
	at: ['Austria'],
	au: ['Australia'],
	az: ['Azerbaijan'],
	ba: ['Bosnia and Herzegovina'],
	bb: ['Barbados'],
	bd: ['Bangladesh'],
	be: ['Belgium'],
	bf: ['Burkina Faso'],
	bg: ['Bulgaria'],
	bh: ['Bahrain'],
	bi: ['Burundi'],
	bj: ['Benin'],
	bn: ['Brunei', 'Brunei Darussalam'],
	bo: ['Bolivia', 'Bolivia (Plurinational State of)'],
	br: ['Brazil'],
	bs: ['Bahamas', 'The Bahamas', 'Bahamas, The'],
	bt: ['Bhutan'],
	bw: ['Botswana'],
	by: ['Belarus'],
	bz: ['Belize'],
	ca: ['Canada'],
	cd: [
		'Democratic Republic of the Congo',
		'Democratic Republic of Congo',
		'Congo, Democratic Republic of the',
		'DRC',
		'Congo Kinshasa',
		'DR Congo',
	],
	cf: ['Central African Republic', 'CAR', 'Central African Rep'],
	cg: [
		'Congo',
		'Congo Brazzaville',
		'Republic of the Congo',
		'Republic of Congo',
		'Congo, Republic of the',
	],
	ch: ['Switzerland'],
	ci: ["Côte d'Ivoire", 'Ivory Coast', 'Cote d Ivoire'],
	cl: ['Chile'],
	cm: ['Cameroon'],
	cn: [
		'China',
		"People's Republic of China",
		'PRC',
		"China, People's Republic of",
	],
	co: ['Colombia'],
	cr: ['Costa Rica'],
	cu: ['Cuba'],
	cv: ['Cabo Verde', 'Cape Verde'],
	cy: ['Cyprus'],
	cz: ['Czechia', 'Czech Republic', 'Czech Rep'],
	de: ['Germany'],
	dj: ['Djibouti'],
	dk: ['Denmark'],
	dm: ['Dominica'],
	do: ['Dominican Republic', 'Dominican Rep', 'Dominican'],
	dz: ['Algeria'],
	ec: ['Ecuador'],
	ee: ['Estonia'],
	eg: ['Egypt'],
	er: ['Eritrea'],
	es: ['Spain'],
	et: ['Ethiopia'],
	fi: ['Finland'],
	fj: ['Fiji'],
	fm: [
		'Micronesia',
		'Federated States of Micronesia',
		'Micronesia (Federated States of)',
	],
	fr: ['France'],
	ga: ['Gabon'],
	gb: [
		'United Kingdom',
		'United Kingdom of Great Britain and Northern Ireland',
		'UK',
	],
	gd: ['Grenada'],
	ge: ['Georgia'],
	gh: ['Ghana'],
	gm: ['Gambia', 'Gambia, The'],
	gn: ['Guinea'],
	gq: ['Equatorial Guinea'],
	gr: ['Greece'],
	gt: ['Guatemala'],
	gw: ['Guinea-Bissau'],
	gy: ['Guyana'],
	hn: ['Honduras'],
	hr: ['Croatia'],
	ht: ['Haiti'],
	hu: ['Hungary'],
	id: ['Indonesia'],
	ie: ['Ireland'],
	il: ['Israel'],
	in: ['India'],
	iq: ['Iraq'],
	ir: ['Iran', 'Islamic Republic of Iran', 'Iran (Islamic Republic of)'],
	is: ['Iceland'],
	it: ['Italy'],
	jm: ['Jamaica'],
	jo: ['Jordan'],
	jp: ['Japan'],
	ke: ['Kenya'],
	kg: ['Kyrgyzstan'],
	kh: ['Cambodia'],
	ki: ['Kiribati'],
	km: ['Comoros'],
	kn: [
		'Saint Kitts and Nevis',
		'St Kitts and Nevis',
		'Saint Kitts',
		'St Kitts',
	],
	kp: ['North Korea', "Korea (Democratic People's Republic of)", 'DPRK', 'NK'],
	kr: ['South Korea', 'Korea, Republic of'],
	kw: ['Kuwait'],
	kz: ['Kazakhstan'],
	la: [
		'Laos',
		"Lao People's Democratic Republic",
		"Lao People's Democratic Rep",
	],
	lb: ['Lebanon'],
	lc: ['Saint Lucia', 'St Lucia'],
	li: ['Liechtenstein'],
	lk: ['Sri Lanka'],
	lr: ['Liberia'],
	ls: ['Lesotho'],
	lt: ['Lithuania'],
	lu: ['Luxembourg', 'Luxemburg'],
	lv: ['Latvia'],
	ly: ['Libya'],
	ma: ['Morocco'],
	mc: ['Monaco'],
	md: ['Moldova', 'Moldova, Republic of'],
	me: ['Montenegro'],
	mg: ['Madagascar'],
	mh: ['Marshall Islands'],
	mk: ['North Macedonia', 'Macedonia'],
	ml: ['Mali'],
	mm: ['Myanmar', 'Burma'],
	mn: ['Mongolia'],
	mr: ['Mauritania'],
	mt: ['Malta'],
	mu: ['Mauritius'],
	mv: ['Maldives'],
	mw: ['Malawi'],
	mx: ['Mexico'],
	my: ['Malaysia'],
	mz: ['Mozambique'],
	na: ['Namibia'],
	ne: ['Niger'],
	ng: ['Nigeria'],
	ni: ['Nicaragua'],
	nl: ['Netherlands'],
	no: ['Norway'],
	np: ['Nepal'],
	nr: ['Nauru'],
	nz: ['New Zealand'],
	om: ['Oman'],
	pa: ['Panama'],
	pe: ['Peru'],
	pg: ['Papua New Guinea'],
	ph: ['Philippines'],
	pk: ['Pakistan'],
	pl: ['Poland'],
	ps: ['Palestine', 'Palestine, State of'],
	pt: ['Portugal'],
	pw: ['Palau'],
	py: ['Paraguay'],
	qa: ['Qatar'],
	ro: ['Romania'],
	rs: ['Serbia'],
	ru: ['Russia', 'Russian Federation'],
	rw: ['Rwanda'],
	sa: ['Saudi Arabia'],
	sb: ['Solomon Islands'],
	sc: ['Seychelles'],
	sd: ['Sudan'],
	se: ['Sweden'],
	sg: ['Singapore'],
	si: ['Slovenia'],
	sk: ['Slovakia'],
	sl: ['Sierra Leone'],
	sm: ['San Marino'],
	sn: ['Senegal'],
	so: ['Somalia'],
	sr: ['Suriname'],
	ss: ['South Sudan'],
	st: ['São Tomé and Príncipe', 'Sao Tome and Principe', 'Sao Tome'],
	sv: ['El Salvador'],
	sy: ['Syria', 'Syrian Arab Republic', 'Syrian Arab Rep', 'Syrian Arab'],
	sz: ['Eswatini', 'Swaziland'],
	td: ['Chad'],
	tg: ['Togo'],
	th: ['Thailand'],
	tj: ['Tajikistan'],
	tl: ['Timor-Leste', 'East Timor'],
	tm: ['Turkmenistan'],
	tn: ['Tunisia'],
	to: ['Tonga'],
	tr: ['Turkey'],
	tt: ['Trinidad and Tobago'],
	tv: ['Tuvalu'],
	tw: ['Taiwan', 'Taiwan, Province of China', 'Taiwan (Republic of China)'],
	tz: ['Tanzania', 'Tanzania, United Republic of'],
	ua: ['Ukraine'],
	ug: ['Uganda'],
	us: ['United States of America', 'United States', 'USA'],
	uy: ['Uruguay'],
	uz: ['Uzbekistan'],
	va: ['Vatican', 'Holy See', 'Vatican City'],
	vc: [
		'Saint Vincent and the Grenadines',
		'Saint Vincent',
		'St Vincent and the Grenadines',
		'St Vincent',
	],
	ve: ['Venezuela', 'Venezuela (Bolivarian Republic of)'],
	vn: ['Vietnam', 'Viet Nam'],
	vu: ['Vanuatu'],
	ws: ['Samoa'],
	xk: ['Kosovo'],
	ye: ['Yemen'],
	za: ['South Africa'],
	zm: ['Zambia'],
	zw: ['Zimbabwe'],
};
